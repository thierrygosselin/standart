...
#'
#' # with defaults
#' read.info <- standart::read_counter(fq.files = "corals")
#'
...
read_counter <- function(
  ...
  if (assertthat::is.string(fq.files) && assertthat::is.dir(fq.files)) {
    fq.files <- standart::list_sample_file(f =  fq.files, full.path = TRUE, recursive = FALSE)
  }
  ...
  if (n.files == 1L) {
    ...
    reads.stats <- stats_standart(data = reads, x = "NUMBER_READS", digits = 0L) %>%
    ...
    if (!is.null(strata)) {
      reads.stats <- stats_standart(data = reads, x = "NUMBER_READS", group.by = "STRATA", digits = 0L) %>%
