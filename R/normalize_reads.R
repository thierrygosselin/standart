...
normalize_reads <- function(
  ...
  fq.type <- standart::fq_file_type(fq.files)
  ...
  if (!is.null(project.info)) {
    ...
    if (length(read.column.name) == 0) {
      ...
      fq.to.normalize <- project.info %>%
        dplyr::filter(INDIVIDUALS %in% standart::clean_fq_filename(basename(fq.files))) %>%
...
# Internal function ------------------------------------------------------------
#' @title standart_normalize
#' @description main normalize function
#' @rdname standart_normalize
#' @export
#' @keywords internal
standart_normalize <- function(
    fq.to.normalize,
    count.reads = FALSE,
    number.replicates,
    sample.reads,
  ...
  clean.names <- standart::clean_fq_filename(basename(fq.to.normalize))
...
write_normalize <- function(number.replicates, subsample.random, fq.to.normalize, sample.reads) {
  ...
  fq.type <- standart::fq_file_type(fq.to.normalize)
