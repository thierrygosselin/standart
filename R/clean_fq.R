...
clean_fq <- function(
  ...
  if (assertthat::is.string(fq.files) && assertthat::is.dir(fq.files)) {
    fq.files <- standart::list_sample_file(f =  fq.files, full.path = TRUE, recursive = TRUE, paired.end = paired.end)
  }
